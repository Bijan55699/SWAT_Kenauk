...
!bmp-ri.out
    ...
    !  write(17,*) 'sub_number;j;curyr;iida;giw_flag(j);',
    ! & 'drain_to(j);dstor;h2oloss;dif;sno_hru(j);snoprev;',
    ! & 'sol_sw(j);swprev;shallst(j);shallstp;deepst(j);',
    ! & 'deepstp;surf_bs(1,j);bsprev;bss(1,j);bssprev;',
    ! & 'qday;latq(j);',
    ! & 'qtile;gw_q(j);revapday;rchrg(j);',
    ! & 'sepbtm(j);tloss;gw_qdeep(j);giw_flin_share_mm;',
    ! & 'etday;ep_day;es_day;giw_evap_out_mm;snoev;giw_vol_mm(j);',
    ! & 'giw_vol_mm_begin;giw_precip_in_mm(j);giw_spillage_out_mm(j);qday'

      write(17,*) 'sub_number;j;curyr;iida;giw_flag;',
     & 'drain_to;dstor;h2oloss;dif;qday;latq;',
     & 'gw_q;gw_qdeep;etday;giw_vol_prev_mm;giw_vol_begin_mm;',
     & 'giw_vol_mm;giw_volx_mm;giw_q_in_mm;giw_precip_in_mm;',
     & 'qiw_spillage_out_mm;giw_evap_out_mm;giw_flin_share_mm',
     & 'giw_seepage_in_mm'
