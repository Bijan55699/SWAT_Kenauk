...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &       * (1.- fimp(urblu(j))) * 1000.) !mm/dt
      ...
      !!computes cn number for day. No concern for GIW routine
      call dailycn

        !! compute runoff - surfq in mm H2O
      if (precipday > 0.1) then

         !!modified so that surfq is always 0 for giw HRUs
         call volq
      ...
      !! qday is calculated here. checks were added to ensure that qday is 0
      !! for giw HRUs (qday is later calculated as spillage for GIW HRUs)
      call surfst_h2o

      !! calculate half-hour rainfall
      if (precipday > 0.01) call alph(0)

      if (qday > 0.0001 .and. giw_flag(j) /= 1) then
        !! compute peak rate - peakr in m3/s

        !!qday should be 0 for GIW hrus so this subroutine will not be called if a GIW hru.
        !!This routine calculates the peakr parameter which may impact transmission loses and
        !!sediment calculations. May need to call after qday is calculated for GIW hrus.
        call pkq(0)
      end if

      if (qday > 0.0001 .and. peakr > 0. .and. giw_flag(j)/=1) then
