...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      character (len=13) :: sdrfile, ltcfile, giwfile
     ...
     &      sub_smfmn(1,i), sub_sftmp(1,i), sub_smtmp(1,i),
     &      sub_timp(1,i)
        do ib = 2, 10
     ...
     &          *  fimp(urblu(ihru))
      ...
      !!GEVENSON
      !!readgiw CALL MOVED BELOW LOOP OVER HRU-SCALE RECORDS AS THE
      !!readgiw CALL REQUIRES HRU AREA INFO SET DURING THE LOOP
      call readgiw(giwfile)
      ...
      if (ils == 2) then
        do j = jj, hrutot(i)
            hru_rufr(ils,ihru) = hru_fr(ihru) * sub_km(i) /
     &          daru_km(i,ils)
        end do
      end if
