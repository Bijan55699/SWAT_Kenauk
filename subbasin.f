...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	...
	if (i_wtrhru == 1 .and. giw_flag(j) /= 1) then  ! calculation only for water HRU
         call water_hru
      else
...
!     &        then
        ...
        !!if a giw hru and surfq(j) is not 0 then throw error
        if(giw_flag(j) == 1 .and. surfq(j) > 0.) then
            write(*,*)"ERROR subbasin.f ~Line 211"
            call Exit(1)
        end if
...
!        end if
        ...
        !!call GIW here
        if(giw_flag(j) == 1) call giw

        if(giw_flag(j) == 1 .and. qday > 0.0001) then

            !!The below subroutines are called in surface.f if a non
            !!giw and qday is > 0. However, qday is not calculated for
            !!giw hrus until the giw subroutine call. So neeed to call
            !!here if a giw hru and qday > 0
            call pkq(0)
            if(peakr > 0.) then
                call tran
                call eiusle
        ...
        !!calculate inflpcp depending on if GIW HRU
        if(giw_flag(j) == 1) then
            if (giw_seepage_out_mm(j) > 0.) then
                inflpcp = giw_seepage_out_mm(j)
            end if
        else
...
!        if (pot_vol(j) < 1.e-6) call etact
     ...
     &    smx(j)) - precipday + qday + latq(j) + sepbtm(j) + qtile
        ...
        if (icfrt(j) == 1 .and. giw_flag(j) == 0) then
          ndcfrt(j) = ndcfrt(j) + 1
          call confert
        end if

        !! apply pesticide in continuous pest operation
        if (icpst(j) == 1 .and. giw_flag(j) == 0) then
          ndcpst(j) = ndcpst(j) + 1
          call conapply
        end if

        !! remove biomass from grazing and apply manure
        if (igrz(j) == 1 .and. giw_flag(j) == 0) then
          ndeat(j) = ndeat(j) + 1
          call graze
        end if
        ...
        !! check for dormancy
        if (igro(j) == 1 .and. giw_flag(j) /= 1) call dormant

        !! compute actual ET for day in HRU
        if (giw_flag(j) == 1) then
            etday = ep_day + es_day + canev + giw_evap_out_mm
        else
...
!        endif
	 ...
	 !! compute reduction in pollutants due to in fixed BMP eff
	   if (bmp_flag(j) == 1) then
        ...
        if (drain_to(j) /= 0) then
            if(qday > 0.) then
                write(*,*)"ERROR subbasin.f 457"
            end if
        end if

        !! compute water yield for HRU
	if(qday < 1.e-6) qday = 0.
	if(latq(j) < 1.e-6) latq(j) = 0.
	if(gw_q(j) < 1.e-6) gw_q(j) = 0.
	if(qtile < 1.e-6) qtile = 0.
	if(gw_qdeep(j) < 1.e-6) gw_qdeep(j) = 0.
...
!       Srini pothole
        ...
        if (giw_flag(j) == 1) call recordgiw
